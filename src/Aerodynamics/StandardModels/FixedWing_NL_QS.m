classdef FixedWing_NL_QS < AerodynamicModel
    %FixedWing_NL_QS
    ...
    methods
        ...
        function obj = FixedWing_NL_QS(Aircraft,Name)
            ...
            % Set the number of dynamic states
            obj.NumberOfStates = 2;

            % check to see if the CSV already exists
            AircraftName = obj.Aircraft.Name;
            AircraftPath = obj.Aircraft.WorkspaceDir;
            if 0 ~= exist([AircraftPath '/AircraftData/' AircraftName '_Parameters_FixedWing_NL_QS.csv'],'file')
                warning(['A FixedWing_NL_QS model already exists for ' AircraftName]);
                return
            end
            
            % Copy the template CSV
            temp = which('AircraftDynamcis-Tools/lib/Parameters_FixedWing_NL_QS.csv');
            [libpath,~,~] = fileparts(temp);
            copyfile([libpath filesep 'Parameters_FixedWing_NL_QS.csv'],[AircraftPath '/AircraftData/' AircraftName '_Parameters_FixedWing_NL_QS.csv']);
        ...
        function [F,M] = Model(obj,eta,vb_r,omega_r,delta,Parameters,Constants)
            ...
            alphadot = 0*alphabetadot(1); % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            betadot = 0*alphabetadot(2);  % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    methods(Static)
        ...
        function obj = loadobj(s)
            ...
            % warning.
            if isstruct(s)
                newObj = FixedWing_NL_QS(s.Aircraft,s.Name); 
                f = fieldnames(s);
                for ii = 1:length(f)
